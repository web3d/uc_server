...
class Command extends Component
{
    ...
    /**
     * @var \ucs\caching\Dependency the dependency to be associated with the cached query result for this command
     * @see cache()
     */
     ...
     * @param \ucs\caching\Dependency $dependency the cache dependency associated with the cached query result.
     ...
     * @param string $type the column type. [[\ucs\db\QueryBuilder::getColumnType()]] will be called
    ...
    protected function queryInternal($method, $fetchMode = null)
    {
        ...
        Uii::info($rawSql, 'ucs\db\Command::query');

        if ($method !== '') {
            $info = $this->db->getQueryCacheInfo($this->queryCacheDuration, $this->queryCacheDependency);
            if (is_array($info)) {
                /* @var $cache \ucs\caching\Cache */
                $cache = $info[0];
                $cacheKey = [
                    __CLASS__,
                    $method,
                ...
                if (is_array($result) && isset($result[0])) {
                    Uii::trace('Query result served from cache', 'ucs\db\Command::query');
                    return $result[0];
                }
            }
        }
        ...
        $token = $rawSql;
        try {
            Uii::beginProfile($token, 'ucs\db\Command::query');
            ...
            Uii::endProfile($token, 'ucs\db\Command::query');
        } catch (\Exception $e) {
            Uii::endProfile($token, 'ucs\db\Command::query');
            throw $this->db->getSchema()->convertException($e, $rawSql);
        }
        ...
        if (isset($cache, $cacheKey, $info)) {
            ...
            Uii::trace('Saved query result in cache', 'ucs\db\Command::query');
        }
