...
use PDO;
use Uii;
use ucs\base\Component;
use ucs\base\InvalidConfigException;
use ucs\base\NotSupportedException;
use ucs\caching\Cache;
 ...
 *
 * ```php
 * $connection = new \ucs\db\Connection([
 *     'dsn' => $dsn,
 *     'username' => $username,
 *     'password' => $password,
 * ]);
...
class Connection extends Component
{
     ...
     */
    public $schemaMap = [
        'pgsql' => 'ucs\db\pgsql\Schema', // PostgreSQL
        'mysqli' => 'ucs\db\mysql\Schema', // MySQL
        'mysql' => 'ucs\db\mysql\Schema', // MySQL
        'sqlite' => 'ucs\db\sqlite\Schema', // sqlite 3
        'sqlite2' => 'ucs\db\sqlite\Schema', // sqlite 2
        'sqlsrv' => 'ucs\db\mssql\Schema', // newer MSSQL driver on MS Windows hosts
        'oci' => 'ucs\db\oci\Schema', // Oracle driver
        'mssql' => 'ucs\db\mssql\Schema', // older MSSQL driver on MS Windows hosts
        'dblib' => 'ucs\db\mssql\Schema', // dblib drivers on GNU/Linux (and maybe other OSes) hosts
        'cubrid' => 'ucs\db\cubrid\Schema', // CUBRID
    ];
    /**
     * @var string Custom PDO wrapper class. If not set, it will use [[PDO]] or [[ucs\db\mssql\PDO]] when MSSQL is used.
     * @see pdo
     */
    public $pdoClass;
    /**
     ...
     * @since 2.0.7
     */
    public $commandClass = 'ucs\db\Command';
    /**
     ...
     * Use 0 to indicate that the cached data will never expire.
     * @param \ucs\caching\Dependency $dependency the cache dependency associated with the cached query results.
     ...
     * @param \ucs\caching\Dependency $dependency the preferred caching dependency. If null, it will be ignored.
    ...
    public function open()
    {
        ...
        try {
            Uii::info($token, __METHOD__);
            Uii::beginProfile($token, __METHOD__);
            $this->pdo = $this->createPdoInstance();
            $this->initConnection();
            Uii::endProfile($token, __METHOD__);
    ...
    {
        if ($this->pdo !== null) {
            Uii::trace('Closing DB connection: ' . $this->dsn, __METHOD__);
            $this->pdo = null;
            $this->_schema = null;
    ...
    protected function createPdoInstance()
    {
        ...
        if ($pdoClass === null) {
            ...
            if (isset($driver)) {
                if ($driver === 'mssql' || $driver === 'dblib') {
                    $pdoClass = 'ucs\db\mssql\PDO';
                } elseif ($driver === 'sqlsrv') {
                    $pdoClass = 'ucs\db\mssql\SqlsrvPDO';
                }
            }
        }
        ...
        if (strncmp('sqlite:@', $dsn, 8) === 0) {
            $dsn = 'sqlite:' . Uii::getAlias(substr($dsn, 7));
        }
    ...
    public function getSchema()
    {
        if ($this->_schema !== null) {
            ...
            if (isset($this->schemaMap[$driver])) {
                ...
                return $this->_schema = Uii::createObject($config);
            } else {
    ...
    protected function openFromPool(array $pool, array $sharedConfig)
    {
        ...
        foreach ($pool as $config) {
            ...
            /* @var $db Connection */
            $db = Uii::createObject($config);

            try {
                $db->open();
                return $db;
            } catch (\Exception $e) {
                Uii::warning("Connection ({$config['dsn']}) failed: " . $e->getMessage(), __METHOD__);
                if ($cache instanceof Cache) {
