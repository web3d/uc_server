...
class Query extends Component implements QueryInterface
{
     ...
     * $query = (new \ucs\db\Query)->from(['u' => 'user', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     * $subquery = (new \ucs\db\Query)->from('user')->where(['active' => true])
     * $query = (new \ucs\db\Query)->from(['activeusers' => $subquery]);
     *
