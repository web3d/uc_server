...
class DbControl extends Control
{
            ...
            if ($delete = $_POST['delete']) {
                if (is_array($delete)) {
                    foreach ($delete as $filename) {
                        @unlink('./data/backup/' . str_replace(array(
                            '/',
                            '\\'
                        ), '', $filename));
                    }
    ...
    function onoperate()
    {
        ...
        if (empty($arr['fileinfo'])) {
            ...
            $this->message($this->_parent_js($appid, 'db_' . $type . '_multivol_redirect', array(
                '$volume' => $arr['fileinfo']['file_num']
            )), 'admin.php?m=db&a=operate&t=' . $type . '&appid=' . $appid . '&nexturl=' . urlencode($arr['nexturl']));
    ...
    function ondelete()
    {
        ...
        if ($next_appid != $appid) {
            $this->message($this->_parent_js($backupdir, 'delete_dumpfile_redirect', array(
                '$appname' => $appname
            )), 'admin.php?m=db&a=delete&appid=' . $next_appid . '&backupdir=' . $backupdir . '&sid=' . $this->sid);
        } else {
